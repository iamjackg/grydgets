...
config_schema = voluptuous.Schema({
    voluptuous.Required('graphics'): {
        voluptuous.Required('fps-limit', default=60): voluptuous.All(int, voluptuous.Range(min=1, max=60)),
        voluptuous.Required('fullscreen', default=False): bool,
        voluptuous.Required('resolution'): voluptuous.All([int, voluptuous.Range(min=1)], voluptuous.Length(2)),
        voluptuous.Optional('fb-device'): str,
        voluptuous.Optional('x-display'): str,
    },
    voluptuous.Required('logging'): {
        voluptuous.Required('level', default='info'): voluptuous.In(['debug', 'info', 'warning'])
    },
})
